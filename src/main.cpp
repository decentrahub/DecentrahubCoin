...
#if defined(NDEBUG)
#error "Decentrahub cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in udcntr) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateDCNTRSupply(int nHeightStart)
{
	...
	uiInterface.ShowProgress(_("Recalculating DCNTR supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating DCNTR supply..."), percent);
		}
	...
	bool* pfMissingInputs, bool fOverrideMempoolLimit, bool fRejectInsaneFee, bool ignoreFees)
{
	...
	if (!CheckTransaction(tx, state, fColdStakingActive))
		return error("%s : transaction checks for of %s failed with %s", __func__, tx.GetHash().ToString(), FormatStateMessage(state));
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 25000 && isPoSActive) {
		nSubsidy = 1 * COIN;
	}
	else if (nHeight <= 50000 && nHeight > 25001) {
		nSubsidy = 1.3 * COIN;
	}
	...
	else if (nHeight <= 7275037 && nHeight > 500000) {
		nSubsidy = 7 * COIN;
	}
	else if (nHeight == 7275038) {
		nSubsidy = 1 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
bool IsSkippingRange(int nHeight) {
	if ((nHeight > (24990) && nHeight < 25050) || (nHeight > 49990 && nHeight < 50050) || (nHeight > 74990 && nHeight < 75050)
		|| (nHeight > 99990 && nHeight < 100050) || (nHeight > 124990 && nHeight < 125050) || (nHeight > 149990 && nHeight < 150050)
		|| (nHeight > 174990 && nHeight < 175050) || (nHeight > 199990 && nHeight < 200050) || (nHeight > 224990 && nHeight < 225050)
		|| (nHeight > 249990 && nHeight < 250050) || (nHeight > 274990 && nHeight < 275050) || (nHeight > 299990 && nHeight < 300050)
		|| (nHeight > 324990 && nHeight < 325050) || (nHeight > 349990 && nHeight < 350050) || (nHeight > 374990 && nHeight < 375050)
		|| (nHeight > 399990 && nHeight < 400050) || (nHeight > 424990 && nHeight < 425050) || (nHeight > 449990 && nHeight < 450050)
		|| (nHeight > 474990 && nHeight < 475050) || (nHeight > 499990 && nHeight < 500050))
	{
		return true;
	}
	return false;
...
{
	util::ThreadRename("decentrahub-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> dcntrInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			dcntrInputs.push_back(stakeIn);
		}
		const bool hasDCNTRInputs = !dcntrInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				...
				if (hasDCNTRInputs) {
					...
					for (const CTxIn& dcntrIn : dcntrInputs)
						if (dcntrIn.prevout == in.prevout)
			...
			while (!chainActive.Contains(prev)) {
				...
				for (const CTransaction& t : bl.vtx) {
					...
					for (const CTxIn& in : t.vin) {
						...
						if (hasDCNTRInputs) {
							for (const CTxIn& stakeIn : dcntrInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == NetMsgType::VERSION) {
		...
		// Decentrahub: We use certain sporks during IBD, so check to see if they are
