...
#include "qt/decentrahub/dashboardwidget.h"
#include "qt/decentrahub/forms/ui_dashboardwidget.h"
#include "qt/decentrahub/sendconfirmdialog.h"
#include "qt/decentrahub/txrow.h"
...
DashboardWidget::DashboardWidget(DecentrahubGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	setCssProperty(ui->labelSquareDcntr, "square-chart-dcntr");
	setCssProperty(ui->labelDcntr, "text-chart-dcntr");
	...
	setCssProperty(ui->labelAmountDcntr, "text-stake-dcntr-disable");
	...
	if (window)
		connect(window, &DecentrahubGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
	if (walletModel && walletModel->getOptionsModel()) {
		...
		connect(ui->pushImgEmpty, &QPushButton::clicked, window, &DecentrahubGUI::openFAQ);
		connect(ui->btnHowTo, &QPushButton::clicked, window, &DecentrahubGUI::openFAQ);
	...
	// update the display unit, to not use the default ("DCNTR")
	updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors()
{
	...
	if (isLightTheme()) {
		...
		gridLineColorX = QColor(18, 22, 11);
		linePenColorY = gridLineColorX;
		backgroundColor = linePenColorY;
	}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			dcntr = (pair.first != 0) ? pair.first / 100000000 : 0;
			chartData->totalDcntr += pair.first;
		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesDcntr.append(dcntr);

		int max = std::max(dcntr, dcntr);
...
void DashboardWidget::onChartRefreshed()
{
	...
	set0->setColor(QColor(140, 194, 36));
	...
	set0->append(chartData->valuesDcntr);

	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalDcntr > 0) {
		setCssProperty(ui->labelAmountDcntr, "text-stake-dcntr");
	}
	else {
		setCssProperty(ui->labelAmountDcntr, "text-stake-dcntr-disable");
	}
	forceUpdateStyle({ ui->labelAmountDcntr });
	ui->labelAmountDcntr->setText(GUIUtil::formatBalance(chartData->totalDcntr, nDisplayUnit));
