...
#include "qt/decentrahub/forms/ui_snackbar.h"
#include "qt/decentrahub/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(DecentrahubGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &DecentrahubGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
