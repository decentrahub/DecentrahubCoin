...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter Decentrahub Coin address (e.g. %1)").arg("DEFi8egT8MKH9xc168wXhkCEPH83t2bSC9"));
...
{
    // return if URI is not valid or is no Decentrahub: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::DCNTR, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert decentrahub:// to decentrahub:
    //
    //    Cannot handle this later, because decentrahub:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::DCNTR, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Decentrahub.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Decentrahub.lnk
...
{
    return GetAutostartDir() / "decentrahub.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a decentrahub.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Decentrahub\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the decentrahub app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add decentrahub app to startup item list
