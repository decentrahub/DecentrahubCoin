...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::DCNTR);
    ...
    if (settings.contains("nAnonymizeDecentrahubAmount") || reset)
        SoftSetArg("-anonymizedecentrahubamount", settings.value("nAnonymizeDecentrahubAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in decentrahub.cpp during shotdown to also remove the window positions
