...
bool CDcntrStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CDcntrStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CDcntrStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CDcntrStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CDcntrStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CDcntrStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CDcntrStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CDcntrStake::GetUniqueness() const
{
    //The unique identifier for a DCNTR stake is the outpoint
...
CBlockIndex* CDcntrStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CDcntrStake::ContextCheck(int nHeight, uint32_t nTime)
{
