...
#else
    const char* pszModule = "decentrahub coin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Decentrahub
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Decentrahub
// Mac: ~/Library/Application Support/Decentrahub
// Unix: ~/.decentrahub
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Decentrahub";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "decentrahub.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty decentrahub.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override decentrahub.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "decentrahubd.pid"));
