...
/**
 * Fees smaller than this (in udcntr) are considered zero fee (for transaction creation)
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
	...
	if (txOut.nValue != GetMNCollateral() * COIN) {
		strError = "Invalid collateral tx value, must be Current collateral in DCNTR (unit of DecentraHub)";
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
	...
	vecSend.push_back(CRecipient{ scriptChange, BUDGET_FEE_TX_OLD, false }); // Old 50 DCNTR collateral

    CCoinControl* coinControl = NULL;
	int nChangePosInOut = -1;
    bool success = CreateTransaction(vecSend, tx, reservekey, nFeeRet, nChangePosInOut, strFail, coinControl, ALL_COINS, useIX, (CAmount)0);
    if (!success) {
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
	std::vector<CRecipient> vecSend;
	vecSend.push_back(CRecipient{ scriptChange, BUDGET_FEE_TX, false }); // New 5 DCNTR collateral
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-decentrahub-address
        ...
        )
{
    ...
    // Parse utxos into CDcntrStakes
    std::list<std::unique_ptr<CStakeInput> > listInputs;
    for (const COutput &out : vCoins) {
        std::unique_ptr<CDcntrStake> input(new CDcntrStake());
